...
struct PrepareQueue {
    ...
    mode: String,
    item_count: usize,
    items: Vec<PrepareQueueItem>,
}

#[derive(Debug, Clone, Serialize)]
struct PrepareQueueItem {
    manifest_id: String,
    raw_path: String,
    kind: String,
    status: String,
...
pub fn execute(custom_kb: Option<&Path>, args: &PrepareArgs) -> Result<()> {
    validate_args(args)?;
...
fn validate_args(args: &PrepareArgs) -> Result<()> {
...
fn prepare_mode(args: &PrepareArgs) -> &'static str {
...
fn load_manifest(kb_path: &Path) -> Result<Manifest> {
    ...
    if !manifest_path.exists() {
        return Err(anyhow!(
            "manifest not found: {}. Run `kb status` first, or run non-dry-run `kb prepare --new` so the manifest can be refreshed.",
...
fn select_entries(
    ...
    if let Some(file) = &args.file {
        ...
        if entry.status == "raw_missing" {
            return Err(anyhow!("cannot prepare missing raw file: {}", entry.path));
        }
    ...
    let mut selected = entries
        .into_iter()
        .filter(is_new_prepare_candidate)
        .collect::<Vec<_>>();
...
fn is_new_prepare_candidate(entry: &ManifestEntry) -> bool {
    if entry.status == "raw_missing" {
        return false;
    }
...
fn build_queue(mode: &str, entries: Vec<ManifestEntry>) -> PrepareQueue {
    let items = entries
        .into_iter()
        .map(|entry| PrepareQueueItem {
            ...
            instructions: prepare_instructions(&entry),
        })
        .collect::<Vec<_>>();

    PrepareQueue {
        schema_version: "0.4.6.1".to_string(),
        generated_by: "kb-cli prepare".to_string(),
...
fn prepare_instructions(entry: &ManifestEntry) -> Vec<String> {
...
fn print_queue_preview(queue: &PrepareQueue) {
    println!("\nPrepare plan preview:");
    println!("  mode       : {}", queue.mode);
    println!("  items      : {}", queue.item_count);
    println!("  generated  : {}", queue.generated_at);

    if queue.items.is_empty() {
        println!("\nNo raw files selected for prepare planning.");
        return;
    }
...
fn write_queue_and_proposal(kb_path: &Path, queue: &PrepareQueue) -> Result<()> {
    fs::create_dir_all(kb_path.join("processing/proposals"))?;

    let queue_path = kb_path.join("processing/prepare_queue.json");
    ...
    if queue.item_count == 0 {
        println!("\nNo raw files currently need prepare planning.");
    } else {
        println!("\nNext steps:");
...
fn render_proposal(queue: &PrepareQueue) -> String {
    let mut out = String::new();
    out.push_str("# Prepare Plan Proposal\n\n");
    out.push_str("This file is generated by `kb prepare`. It is a review artifact, not an automatic wiki edit.\n\n");
    ...
    if queue.items.is_empty() {
        out.push_str("No raw files were selected for prepare planning.\n");
        return out;
    }
...
fn render_agent_prompt(queue: &PrepareQueue) -> String {
    ...
    out.push_str("## Prepare scope\n\n");
    out.push_str(&format!("- Mode: `{}`\n", queue.mode));
    out.push_str(&format!("- Generated at: `{}`\n", queue.generated_at));
    out.push_str(&format!("- Item count: `{}`\n\n", queue.item_count));

    if queue.items.is_empty() {
        out.push_str("There are no raw files to prepare in this queue. Do not edit the wiki.\n");
        return out;
    }

    out.push_str("## Source files to prepare\n\n");
    for item in &queue.items {
        out.push_str(&format!("### `{}`\n\n", item.raw_path));
        out.push_str(&format!("- Kind: `{}`\n", item.kind));
        out.push_str(&format!("- Status: `{}`\n", item.status));
