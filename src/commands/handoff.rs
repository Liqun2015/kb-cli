...
5. Do not modify `raw/`.
6. Do not mark relations as `confirmed` unless explicitly allowed.
7. End with changed files, unresolved issues, and review needs.
...
Claude Code should work as an external LLM Wiki agent, not as an invisible automatic knowledge generator.
...
OpenCode should read the generic LLM Wiki handoff first and then operate on one bounded task.
...
OpenClaw-like personal assistant agents may have broader service access than a code-only agent. For LLM Wiki work, keep the scope narrow.
...
5. Keep candidate relations as candidate unless the task explicitly allows confirmation.
6. At the end, list changed files and unresolved review points.
```
"#
    )
}
...
fn print_handoff_report(report: &HandoffReport) {
    ...
    println!("  kb path  : {}", report.kb_path);
