...
pub struct TasksArgs {
    #[arg(long, help = "Preview deferred task scan without writing LLM/tasks/*.md")]
    pub dry_run: bool,

    #[arg(long, help = "Alias for --dry-run")]
    pub preview: bool,

    #[arg(long, default_value_t = 0, help = "Maximum number of task groups to return. Use 0 for no limit.")]
    pub limit: usize,
    ...
    for pdf in collect_files_with_extensions(&raw_papers, &["pdf"])? {
        ...
        if missing || empty {
            ...
            evidence.push(format!(
                ...
                if missing { "missing extracted text" } else { "empty extracted text" }
            ));
        }
    }
...
fn scan_reference_reconciliation_tasks(kb_path: &Path) -> Result<Option<DeferredTask>> {
    ...
    let heading_re = Regex::new(r"(?i)^\s*(references|bibliography|works cited|参考文献|参考资料|文献)\s*$")?;
    ...
    for text_file in collect_files_with_extensions(&text_dir, &["txt", "md"])? {
        ...
        for (line_idx, line) in content.lines().enumerate() {
            ...
            if heading_re.is_match(trimmed) || doi_re.is_match(trimmed) || numbered_entry_re.is_match(trimmed) {
                local_hits += 1;
                if evidence.len() < 20 {
                    evidence.push(format!(
                        "{}:{}: {}",
...
fn scan_bibliographic_index_review_tasks(kb_path: &Path) -> Result<Option<DeferredTask>> {
    ...
    for report in collect_files_with_extensions(&refs_dir, &["md"])? {
        ...
        for line in content.lines() {
            if line.contains("| `candidate` |") || line.contains("| `ambiguous` |") || line.contains("| `missing` |") {
                local_hits += 1;
                if evidence.len() < 20 {
                    evidence.push(format!(
                        "{}: {}",
    ...
    Ok(Some(DeferredTask {
        ...
        goal: "Review uncertain bibliographic index relation candidates generated by kb refs-index.".to_string(),
        requirements: vec![
            "Confirm, reject, or mark missing each candidate / ambiguous / missing relation.".to_string(),
...
fn scan_keyword_topic_relation_tasks(kb_path: &Path) -> Result<Option<DeferredTask>> {
    ...
    for report in collect_files_with_extensions(&keywords_dir, &["md"])? {
        ...
        let mut local_hits = 0usize;
        for line in content.lines() {
            if line.contains("| `candidate` |") || line.contains("keyword/topic relation") || line.contains("shared terms") {
                local_hits += 1;
                if evidence.len() < 20 {
                    evidence.push(format!(
                        "{}: {}",
...
fn scan_wiki_source_traceability_tasks(kb_path: &Path) -> Result<Option<DeferredTask>> {
    ...
    for page in collect_files_with_extensions(&wiki_dir, &["md"])? {
        ...
        if !content.contains("source_files:") && !content.contains("source_ids:") {
            ...
            evidence.push(format!("{rel}: missing source_files/source_ids front matter"));
        }
    }
        ...
        for (line_idx, line) in content.lines().enumerate() {
            for captures in wikilink_re.captures_iter(line) {
                ...
                if !page_keys.contains(&normalize_page_key(&target)) {
                    ...
                    if evidence.len() < 40 {
                        evidence.push(format!("{}:{}: broken link [[{}]]", rel, line_idx + 1, target));
                    }
                }
            }
        }
