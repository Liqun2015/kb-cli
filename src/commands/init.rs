...
///
/// v0.7.10 prefers the lowercase `knowledgebase/` workspace directory,
...
pub fn execute_with_source(
    ...
    create_interfaces_readme(&kb_path, force)?;
...
fn create_interfaces_readme(kb_path: &Path, force: bool) -> Result<()> {
    let readme_path = kb_path.join("interfaces/README.md");
    if readme_path.exists() && !force {
        println!("[SKIPPED] interfaces/README.md (already exists)");
        return Ok(());
    }
...
- `html/` — browser review pages generated by `kb check`
...
"#,
    ...
    println!("[CREATED] interfaces/README.md");
    Ok(())
}
...
├── interfaces/          # Interface artifacts
│   ├── html/         # Browser review pages generated by kb check
│   ├── reports/      # Lint/health/audit reports
│   ├── logs/         # Generated logs and metadata
...
Rules:
- `source_ids` should contain manifest IDs from `processing/manifest.json` or topic task files when available.
...
This schema is generated by `kb-cli v0.7.10`. It is intentionally simple and should evolve with the project.
"#
}
...
```text
interfaces/reports/lint_static_YYYYMMDD_HHMMSS.md
```
"#
}
