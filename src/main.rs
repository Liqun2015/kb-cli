...
fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        Some(Commands::Init) => commands::init::execute(cli.kb_path.as_deref(), false),
        Some(Commands::BuildWiki) => commands::build_wiki::execute(cli.kb_path.as_deref()),
        Some(Commands::ExtractMetadata) => commands::extract_metadata::execute(cli.kb_path.as_deref(), false),
