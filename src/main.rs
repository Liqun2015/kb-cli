...
enum Commands {
    ...
    #[command(about = "Scan wiki Markdown pages for WikiLinks and deterministic link-resolution hints")]
    ...
    #[command(about = "Scan extracted text for reference headings, DOI values, and citation-pattern hints")]
    Refs(commands::refs::RefsArgs),
    #[command(about = "Build bibliographic index relation candidates between reference entries and local papers")]
    RefsIndex(commands::refs_index::RefsIndexArgs),
    #[command(about = "Export bibliographic relation candidates as graph data for third-party visualization")]
    ...
    #[command(about = "Extract plain text from supported source files with deterministic best-effort methods")]
    ExtractText(commands::extract_text::ExtractTextArgs),
    #[command(name = "shell", alias = "repl", about = "Enter deterministic interactive shell mode")]
    Shell,
...
fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Some(Commands::RefsIndex(args)) => commands::refs_index::execute(cli.kb_path.as_deref(), args),
        Some(Commands::RefsGraph(args)) => commands::refs_graph::execute(cli.kb_path.as_deref(), args),
        ...
        Some(Commands::ExtractText(args)) => commands::extract_text::execute(cli.kb_path.as_deref(), args),
        ...
        None => {
            ...
            println!("  shell                         Enter deterministic interactive command shell");
