...
The shell handles only session commands directly (`use`, `pwd`, `clear`, `help`, `exit`). Known knowledge-base commands are delegated back to the same batch-mode `kb` executable with the current `--wiki` value. This preserves one-to-one command semantics without creating a separate business-logic stack.
