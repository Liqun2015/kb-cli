...
This document records what `kb-cli v0.4.5` actually supports. It is intentionally conservative.
...
`kb status` is implemented in v0.4.5:
...
`kb compile` is implemented in v0.4.5 as a planning command only. It may write:
...
`kb lint-static` is implemented in v0.4.5 as a deterministic local Markdown health check. It does not call an LLM and does not rewrite `wiki/`.
...
Do not assume the following exist in `v0.4.5`:
