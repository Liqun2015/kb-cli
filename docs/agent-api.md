...
| `kb check [--no-open|--dry-run|--output-dir DIR]` | Generate the regular static HTML dashboard. |
| `kb check --relations [--topic TOPIC] [--data-only]` | Generate the topic relation overview, or a strict single-topic graph when `--topic` is provided. |
...
`kb check --relations` is the topic relation overview. It lists topic workspaces under `topics/`. `kb check --relations --topic <topic>` is a strict single-topic graph and must not include unrelated topics in `relationship_data.json`.
