...
## Manager LLM task intake

Starting in `v0.7.3`, the Manager LLM should begin task intake from:

```text
LLM/tasks/index.md
```

This file is generated by `kb tasks` and points to bounded Worker task files under:

```text
LLM/tasks/items/<task_id>.md
```

The Manager LLM should assign one item at a time to a Worker LLM or human reviewer. Worker LLMs should not browse the whole task directory looking for work.
