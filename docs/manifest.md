...
`kb-cli v0.5.10` maintains a lightweight raw-file registry:
...
It is also refreshed automatically after a non-dry-run `kb ingest` and during `kb setup`.
...
kb --kb-path /path/to/literature-folder/knowledgebase status --preview
```
