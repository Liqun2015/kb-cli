...
`prepare` is implemented only as a review-first planning command in v0.4.7. It generates `processing/prepare_queue.json` and `processing/proposals/prepare_plan_*.md`; it does not call an LLM or edit `wiki/`. `query` is still planned; `lint-static` is implemented as a deterministic local health check.
